...
import { countActiveComparisonFilters, type ComparisonFilters } from "./comparison-filters";
import { ComparisonFiltersSidebar } from "./ComparisonFiltersSidebar";
import type { SavedComparisonFilter } from "./filters-storage";

interface ComparisonFilterSheetProps {
  open: boolean;
...
export function ComparisonFilterSheet({
  open,
  onOpenChange,
  filters,
  onChange,
  ...
  onApplyFilter,
  onDeleteFilter,
}: ComparisonFilterSheetProps) {
  const active = countActiveComparisonFilters(filters);
        ...
        <SheetHeader className="border-b border-border bg-gradient-to-r from-card to-primary/5 px-4 py-4">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <SheetTitle className="text-base">Filtres comparaisons</SheetTitle>
                {active > 0 && (
...
export function ComparisonFilterTrigger({
  activeCount,
  onClick,
}: {
  activeCount: number;
