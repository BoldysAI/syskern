...
import { countActiveOfferFilters, type OfferFilters } from "./offer-filters";
import { OffersFiltersSidebar } from "./OffersFiltersSidebar";
...
export function OffersFilterSheet({
  ...
  const active = countActiveOfferFilters(filters);
        ...
        <SheetHeader className="border-b border-border bg-gradient-to-r from-card to-primary/5 px-4 py-4">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <SheetTitle className="text-base">Filtres offres</SheetTitle>
                {active > 0 && (
...
export function OffersFilterTrigger({
  activeCount,
  onClick,
}: {
  activeCount: number;
