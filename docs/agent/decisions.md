...
## 2026-07-22 · [T] Odoo — planification et politique d'erreurs : Celery Beat + `_rpc.py`, ni apscheduler ni tenacity

Requalification de tâches Notion qui semblaient « à faire » alors que le besoin CDC est couvert
autrement. Les écarts existaient dans le code mais n'étaient **pas tracés** — c'est corrigé ici
(cf. AGENTS §11 : une déviation non documentée est une dérive).

- **Sync quotidienne (CDC §5.4.1)** — le ticket dit « job apscheduler 03:00 UTC ». La stack est
  **Celery + django-celery-beat** (AGENTS §3), donc : `PeriodicTask` « Daily Odoo full sync
  (03:00 UTC) » → `odoo_sync.sync_task` (migration `odoo_sync/0002_daily_sync_periodic_task`).
  Même déviation que pour les crons `documents`/`offers` déjà tracés. **apscheduler n'est pas une
  dépendance du projet et ne doit pas l'être.**
- **Delta `write_date`** : implémenté côté adapters — `domain.append(["write_date", ">=", …])`
  quand `modified_since` est fourni (v16 et v19).
- **Stratégie retry / timeout / backoff (CDC §5.5)** — le ticket dit « tenacity ». **tenacity n'est
  pas dans `pyproject.toml`** et n'a pas besoin d'y être : la politique est **centralisée** dans
  `apps/odoo_sync/adapters/_rpc.py` (docstring « Retry policy (CDC §5.5) ») et partagée par les deux
  adapters — 3 tentatives avec backoff exponentiel 2/4/8 s sur 5xx et erreurs réseau, **pas de retry
  sur 4xx** (corrigeable par l'appelant), ré-authentification unique puis rejeu sur expiration de
  session. Timeout explicite sur chaque client httpx (Odoo, Gamma 60 s). Au niveau tâche, le push
  produit ajoute `autoretry_for` + `retry_backoff` (max 3) et un balayage horaire
  `odoo_sync.retry_failed_product_pushes`. **Ne pas ajouter tenacity** pour « se conformer au
  libellé » : ce serait une dépendance redondante.
- **Sync à la demande (CDC §5.4.3)** : `ProductViewSet.perform_create` → `push_product_async` →
  `odoo_sync.push_product_task`, échecs rattrapés par le balayage horaire.

**Restent réellement ouverts** (ne pas basculer en « To test ») :
- **Tests E2E** : 71 fichiers de tests backend (unitaires + intégration, dont l'exemple de pricing
  CDC §6.4) mais **aucun framework de test frontend** (ni Playwright ni Cypress dans
  `frontend/package.json`). Le volet E2E de l'epic QA-DOC n'est pas couvert.
- **Bucket Supabase Storage `offer-documents` + RLS** : **sans objet** depuis la décision du
