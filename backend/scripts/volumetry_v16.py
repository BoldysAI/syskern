...
for batch_size in [100, 500, 1000]:
    ...
    print(
        f"  search_read fields={len(FIELDS)} limit={batch_size:4d} -> {len(records):4d} rows in {dt:5.2f}s "
...
while True:
    batch = kw(
        "product.template", "search_read", [[]], {"fields": FIELDS, "limit": 500, "offset": offset}
    )
    all_records += batch
    if len(batch) < 500:
        break
