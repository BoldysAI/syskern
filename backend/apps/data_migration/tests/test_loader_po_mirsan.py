...
def _generate_mirsan_fixture(path: Path) -> None:
    ...
    for _ in range(5):
        ws.append([None] * 25)
    ws.append(["Mirsan Product Code", "ukn products codes ", "ukn number codes",
               "Dimensions", "Designation "] + ["Unit Price"] * 11 +
              ["SYMEA DDP Prices", "SYMEA DDP Prices", None,
               "SYSKERN DDP Prices", "SYSKERN DDP Prices", None, "Ex Works Eskisehir"])
    ws.append(["MR.HD.GTN22U66.01_STRS", "KRS2266SGR", "U1402001",
               "22RU 600mmx600mm", "Single glass door - Rear panel"] +
              [None] * 11 + [124.31, None, None, 132.25, None, None, None])
    ws.append(["MR.HD.GTN24U66.01_STRS", "KRS2466SGR", "U1402002",
               "24RU 600mmx600mm", "Single glass door - Rear panel"] +
              [None] * 11 + [133.0, None, None, 141.0, None, None, 158.0])
    # Blank row (section break)
    ws.append([None] * 25)
    ws.append(["MR.HD.GTN42U66.01_STRS", "KRS4266SGR", "U1402005",
               "42RU 600mmx600mm", "Single glass door - Rear panel"] +
              [None] * 11 + [184.0, None, None, 196.0, None, None, 205.0])

    # GRID CABINETS — header_row=4 (0-based), price in col 13
    ws2 = wb.create_sheet("GRID CABINETS")
    for _ in range(4):
        ws2.append([None] * 20)
    ws2.append(["Mirsan Product Code", "ukn products codes ", "ukn number codes",
                "Dimensions", "Door Configurations"] + ["Unit Price"] * 4 +
               ["TG", "Unit Price", "SYMEA DDP Prices", None, "SYSKERN DDP Prices"])
    ws2.append(["MR.HD.GTN22U66G.01_GRID", "KRG2266SGR", "U1403001",
                "22RU 600mmx600mm", "Single Front Glass Door - Full rear",
                193.1, None, None, None, 161,
                None, 151.4, None, 161.1])
    ws2.append(["MR.HD.GTN24U66G.01_GRID", "KRG2466SGR", "U1403002",
                "24RU 600mmx600mm", "Single Front Glass Door - Full rear",
                201.7, None, None, None, 172,
                None, 165.0, None, 175.5])

    # RACKS & OPEN RACKS — header_row=4 (0-based), UKN in col 1, price in col 13
    ws3 = wb.create_sheet("RACKS & OPEN RACKS")
    for _ in range(4):
        ws3.append([None] * 16)
    ws3.append(["Product Description", "ukn products codes ", "ukn number codes",
                "Mirsan Code"] + ["SYMEA PRICES"] * 5 + ["TG"] + ["SYMEA PRICES"] * 3 +
               [None, "SYSKERN DDP Prices"])
    ws3.append(["691707IP55.2", "KWI07600", "-", "MR.IP55W07U60.03",
                166.93, 136.85, None, None, "$120,00", 120.0,
                None, 102.92, None, 109.49])
    ws3.append(["691709IP55.2", "KWI09600", "-", "MR.IP55W09U60.03",
                175.34, 152.95, None, None, "$130,00", 130.0,
                None, 111.49, None, 118.61])

    # ACCESSORIES 19 — header_row=3 (0-based), UKN in col 2, price in col 9
    ws4 = wb.create_sheet("ACCESSORIES 19")
    for _ in range(3):
        ws4.append([None] * 12)
    ws4.append(["MIRSAN CODE", "DESIGNATION / TYPE", "ukn products codes ",
                "ukn number codes", "Product Description"] +
               ["EX-WORKS PRICES ESKISEHIR"] * 4 + ["SYMEA DDP Prices"])
    ws4.append(["MR.FAN2AT.XX", "FAN BLOCK 2V", "KFAN2V", "ACC001",
                "2 fans analog thermostat module",
                30.25, 30.25, 25.0, 25.0, 21.44])
    ws4.append(["MR.FAN4AT.XX", "FAN BLOCK 4V", "KFAN4V", "ACC002",
                "4 fans analog thermostat module",
                43.45, 43.45, 40.0, 40.0, 34.31])
...
class TestReadSheetAsDf:
    ...
    def test_sheet_not_found_returns_empty(self) -> None:
        ...
        missing = _SheetProfile(
            ...
            header_row=0, sku_col=0, item_code_col=1, desc_col=2,
            mirsan_col=3, dim_col=4, price_col=5,
        )
...
class TestMirsanMatchHint:
    def test_sku_code_in_hint(self) -> None:
        loader = MirsanLoader()
        raw = pd.Series({"sku_code": "KRS2266SGR", "price_eur": "132", "item_code": None,
                         "description": "desc", "mirsan_code": None, "dimensions": None, "__sheet__": "START CABINETS"})
